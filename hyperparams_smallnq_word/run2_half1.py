...
lrs = [9.70625888478979e-05, 0.0008497018999399376, 9.837082062376434e-05, 9.849150152030287e-05] # 0.005229962664012459, 0.007245059728952319, 1.4891629034786948e-06, 0.00031275436838045526
bss = [128, 8, 8, 8] # 8, 16, 8, 64

for lr, bs in zip(lrs, bss):

    modelPath = "hyperparams_smallnq_word/run1/model_bs" + str(bs) + "_lr" + str(lr)
    os.mkdir(modelPath)
    os.mkdir(modelPath + "/pickles")
    os.mkdir(modelPath + "/tf")

    optimizer = tf.compat.v1.train.AdamOptimizer(lr).minimize(logloss)
    saver = tf.compat.v1.train.Saver(max_to_keep=29)
